...
def test_find_peaks(tissue_object):
    ...
    assert np.all([round(amplitude, 3) == 3.000 for amplitude in tissue_object.peaks[1]])
...
def test_find_analysispoints(tissue_object):
    ...
    assert np.all([round(point, 2) == 1.200 for point in tissue_object.contract_points[0][1]])
    assert np.all([round(point, 2) == 1.400 for point in tissue_object.contract_points[1][1]])
    assert np.all([round(point, 2) == 2.000 for point in tissue_object.contract_points[2][1]])
    assert np.all([round(point, 2) == 2.600 for point in tissue_object.contract_points[3][1]])
    assert np.all([round(point, 2) == 2.800 for point in tissue_object.contract_points[4][1]])

    assert np.all([round(point, 2) == 1.200 for point in tissue_object.relax_points[0][1]])
    assert np.all([round(point, 2) == 1.400 for point in tissue_object.relax_points[1][1]])
    assert np.all([round(point, 2) == 2.000 for point in tissue_object.relax_points[2][1]])
    assert np.all([round(point, 2) == 2.600 for point in tissue_object.relax_points[3][1]])
    assert np.all([round(point, 2) == 2.800 for point in tissue_object.relax_points[4][1]])
