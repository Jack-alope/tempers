...
def carry_calcs(db, all_data, files):
    ...
    for i, file in enumerate(files):
        ...
        if bio_object.bio_reactor_number != 0:
            # HACK: fix this so the form starts at zero or find a better way
            loc = tissue_object.post_number - 1
            print(loc)
    ...
    output(db, actforce, devforce, pasforce, beat_freq, freqCOV, time2pks, time50rel, time80rel, time90rel, t_50, c_50, r_50, slope,
           negslope, path, files)
    return
...
def output(db, actforce, devforce, pasforce, beat_freq, freqCOV, time2pks, time50rel, time80rel, time90rel, t_50, c_50, r_50, slope,
           negslope, path, files):
    ...
    for i in range(len(t_50)):
        tissue_object = crud_tissue.get_tissue_by_csv(db, files[i])
