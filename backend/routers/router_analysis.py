...
async def analyze(
    video_id: Optional[int] = Query(None), database: Session = Depends(get_db)
):
...
async def analyze_tissue_number(
    tissue_number: int = Query(...),
    ...
    if date_recorded:
        tissues_with_freq = (
            crud_tissue.get_tissues_by_experiment_and_tissue_number_and_date_recorded(
...
def graph_update(
    data: schema_analysis.AnalysisBase, database: Session = Depends(get_db)
):
    ...
    if data.video_id:
        ...
        tissue_with_freq = (
            crud_tissue.get_tissues_by_experiment_and_tissue_number_and_date_recorded(
                database, data.experiment_id, data.tissue_number, data.date_recorded
            )[data.value]
        )
        ...
        tissue_with_freq = crud_tissue.get_tissues_by_experiment_and_tissue_number(
            database, data.experiment_id, data.tissue_number
        )[data.value]
        # the tissue object is in spot zero of the tuple frew in spot 1
        tissue_obj = tissue_with_freq[0]
    ...
    tracking_obj.smooth(
        int(data.windows), int(data.polynomials), data.buttons[0], data.buttons[1]
    )
    tracking_obj.find_peaks(
        float(data.thresholds), int(data.minDistances), data.xrange, int(data.buffers)
    )
    ...
    crud_tissue_tracking.update_forces_disp(
        database,
        tissue_obj.id,
        tracking_obj.smooth_force,
        tracking_obj.raw_force,
        tracking_obj.smooth_disp,
    )

    contractx = (
        tracking_obj.contract_points[0][0].tolist()
        + tracking_obj.contract_points[2][0].tolist()
        + tracking_obj.contract_points[4][0].tolist()
    )
    contracty = (
        tracking_obj.contract_points[0][1].tolist()
        + tracking_obj.contract_points[2][1].tolist()
        + tracking_obj.contract_points[4][1].tolist()
    )
    relaxx = (
        tracking_obj.relax_points[0][0].tolist()
        + tracking_obj.relax_points[1][0].tolist()
        + tracking_obj.relax_points[2][0].tolist()
        + tracking_obj.relax_points[4][0].tolist()
    )
    relaxy = (
        tracking_obj.relax_points[0][1].tolist()
        + tracking_obj.relax_points[1][1].tolist()
    ...
    return {
        "status": "OK",
        "data": {
            "xs": tracking_obj.time,
            "ys": tracking_obj.smooth_force.tolist(),
            "peaksx": tracking_obj.peaks[0],
            "peaksy": tracking_obj.peaks[1],
            "basex": tracking_obj.basepoints[0],
            "basey": tracking_obj.basepoints[1],
            "frontx": tracking_obj.frontpoints[0],
            "fronty": tracking_obj.frontpoints[1],
            "contractx": contractx,
            "contracty": contracty,
            "relaxx": relaxx,
            "relaxy": relaxy,
            "dfdtx": tracking_obj.time,
            "dfdty": tracking_obj.dfdt.tolist(),
            "rawx": tracking_obj.time,
            "rawy": tracking_obj.raw_force.tolist(),
        },
    }
...
def download_summary(
    video_id=Query(None),
    experiment_identifier=Query(None),
    tissue_number=Query(None),
    ...
    response = StreamingResponse(
        io.StringIO(caculations_df.to_csv(index=False)), media_type="text/csv"
    )
