...
async def tissues_in_experiment(
    experiment_id: str = Query(...), database: Session = Depends(get_db)
):
    """Returns tissues within an experiment"""
    return crud_tissue.get_tissue_numbers_in_experiment(database, experiment_id)


@router.get("/tissues_in_experiment_by_date_recorded", tags=["tissues"])
async def tissues_in_experiment_by_date(
