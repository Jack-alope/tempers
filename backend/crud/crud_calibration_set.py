...
def get_calibration_sets_by_li_identifier(database_session: Session,
                                          calibration_set_ids: List[str]):
    return database_session.query(models.CalibrationSet).options(
        noload(models.CalibrationSet.videos)).filter(
            models.CalibrationSet.calibration_set_identifier.in_(
                calibration_set_ids)).all()
