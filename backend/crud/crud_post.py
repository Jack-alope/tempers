...
def create_post(
    database_session: Session, post: schema_post.PostCreate, bio_reactor_id: int
):
    """Create Post"""
...
def get_post_by_num_and_bio(
    database_session: Session, bio_reactor_id: int, post_number: int
):
    """Get post object by number and bio number"""
    return (
        database_session.query(models.Post)
        .filter(
            models.Post.bio_reactor_id == bio_reactor_id,
            models.Post.post_number == post_number,
        )
        .first()
    )
...
def get_posts_by_bio_id(database_session: Session, bio_reactor_id: int):
    ...
    return (
        database_session.query(models.Post)
        .filter(models.Post.bio_reactor_id == bio_reactor_id)
        .all()
    )
