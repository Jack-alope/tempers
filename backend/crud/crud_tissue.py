...
def get_tissue_by_id(database_session: Session, tissue_id: int):
    """Get tissue by id"""
    return (
        database_session.query(models.Tissue)
        .filter(models.Tissue.id == tissue_id)
        .first()
    )
...
def get_tissue_numbers_in_experiment(database_session: Session, experiment_id: int):
    ...
    tissues = (
        database_session.query(models.Tissue)
        .join(models.Video)
        .filter(models.Video.experiment_id == experiment_id)
        .distinct()
        .all()
    )
...
def get_tissue_numbers_in_experiment_on_date(
    database_session: Session, experiment_id: int, date_recorded: date
):
    """Returns tissues within an experiment"""
    tissues = (
        database_session.query(models.Tissue)
        ...
        .filter(
            ...
            models.Video.date_recorded == date_recorded,
        )
        .distinct()
        .all()
    )
...
def get_tissues_by_experiment_and_tissue_number(
    database_session: Session, experiment_id: str, tissue_number: int
):
    """Returns the tissues within an experiemnt with the same tissue number"""

    return (
        database_session.query(models.Tissue, models.Video.frequency)
        .join(models.Video)
        .filter(
            models.Video.experiment_id == experiment_id,
            models.Tissue.tissue_number == tissue_number,
        )
        .order_by(models.Tissue.id)
        .all()
    )


def get_tissues_by_experiment_and_tissue_number_and_date_recorded(
